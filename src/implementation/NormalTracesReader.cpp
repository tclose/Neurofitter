...
ModelResults NormalTracesReader::readTracesFromFilesVector(
    ...
    ModelResults results(
            getNumberOfRuns() * getNumberOfPeriods()
                    * getNumberOfRecordSites());
        ...
        for (int nPeriod = 0; nPeriod < numberOfPeriods; nPeriod++) {
            ...
            for (int nRecording = 0; nRecording < numberOfRecordSites;
                ...
                results[periodIndex + nRecording].setName(
                        "Run: " + str(nRun) + ", Period: " + str(nPeriod)
                                + ", Recording: " + str(nRecording));
                ...
                results[periodIndex + nRecording].setStopTime(
                        periodStops[nPeriod]);
                results[periodIndex + nRecording].setLag(
                        differenceLags[nPeriod], differenceWeights[nPeriod]);
                ...
                results[periodIndex + nRecording].setNumberOfPeriods(
                        numberOfPeriods);
...
ModelResults NormalTracesReader::readTracesFromDirectory(string dirName) {
    ...
    for (unsigned int i = 0; i < filesVector.size(); i++) {
        filesVector[i] = dirName + "/" + fixedParams["OutputFilePrefix"]
                + "_Run" + str(i) + ".dat";
    }
